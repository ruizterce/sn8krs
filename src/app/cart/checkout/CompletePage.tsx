...
export default function CompletePage() {
  ...
  return (
    <div id="payment-status" className="flex flex-col items-center gap-4">
      ...
      <h2 id="status-text" className="text-3xl font-futuraExtraBold">
        {STATUS_CONTENT_MAP[status as keyof typeof STATUS_CONTENT_MAP].text}
      </h2>
      {intentId && (
        <div
          id="details-table"
          className="w-full max-w-md overflow-hidden drop-shadow-md-h bg-background"
        >
          <table className="table-auto w-full border-collapse border border-gray-200">
            <tbody>
              <tr className="border-b border-gray-200">
                <td className="px-4 py-2 font-medium text-gray-600">id</td>
                <td id="intent-id" className="px-4 py-2 text-gray-800">
                  {intentId}
                </td>
              </tr>
              <tr>
                <td className="px-4 py-2 font-medium text-gray-600">status</td>
                <td id="intent-status" className="px-4 py-2 text-gray-800">
                  {status}
                </td>
              </tr>
            </tbody>
